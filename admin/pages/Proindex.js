import Head from 'next/head'
import Image from 'next/image'
import { Inter } from '@next/font/google'
import styles from '../styles/Home.module.css'
import {MdOutlineSystemUpdateAlt} from 'react-icons/bi';
import {BiCartAlt} from 'react-icons/bi';
import ProTable from'../components/Protable';
import ProForm from'../components/proform';
import { useState } from 'react';
import Link from 'next/link';


export default function Proindex() {

  const[visible,setVisible]=useState(false)

  const handler=()=>{
    setVisible(!visible)
  }

  return (
    <section>
      <Head>
        <title>Admin Product Management</title>
        <meta name="description" content="Generated by create next app" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/clipart586690.png"/>
      </Head>

      <main className='py-5'>
    
      <h1 className='text-xl md:text-5xl text-center font-bold'>Admin Management</h1>
      

      <div className="container mx-auto flex justify-between py-5 bordor-b">
        <div className="left flex gap-3">
          <button onClick={handler} className="flex bg-green-500 text-white px-4 py-2 border rounded-md hover:bg-gray-50 hover:border-lime-500 hover:text-gray-800">
           Add Management<span className='px-1'></span>
          </button>
        </div>
        {/* <div className="left flex gap-3">
          <button onClick={handler} className="flex bg-yellow-500 text-white px-4 py-2 border rounded-md hover:bg-gray-50 hover:border-lime-500 hover:text-gray-800">
            Update Product<span className='px-1'></span>
          </button>
        </div> */}
        <div className="left flex gap-3">
        <Link href="/AdminPages" className="flex bg-blue-500 text-white px-4 py-2 border rounded-md hover:bg-gray-50 hover:border-lime-500 hover:text-gray-800">
        User Management<span className='px-1'><BiCartAlt size={24}></BiCartAlt></span>
          </Link>
        </div>
      </div>
        {/*collapsable from */}
        {visible?<ProForm></ProForm>:<></>}
        

        {/* {table} */}
        <div className="container mx-auto">
          <ProTable></ProTable>
        </div>
        
    </main>
    </section>
  )
}
